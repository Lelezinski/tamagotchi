...
/******************************************************************************
* Function Name  : GUI_Draw
*******************************************************************************/

void GUI_Draw(const uint8_t *imgPtr, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t pixArea)
{
	uint32_t index = 0;
	...
	for (index = 0; index < (width * height * pixArea); index += 2)
	{
		// Reset row if end of line
		if ((index % width) == 0)
		{
			LCD_SetCursor(x, y++);
			LCD_WriteIndex(0x0022);
			// When pixel row is done, increment index
			if ((index % (width * pixArea)) == 0)
			{
				row_index += width;
			}
		}
		...
		// Grabs RGB565 value
		ind = (index % width) + row_index;
		currRGB = (imgPtr[ind] << 8 | imgPtr[ind + 1]);

		// Single Pixel Draw
		for (pixel_index = 0; pixel_index < pixArea; pixel_index++)
		{
			LCD_WriteData(currRGB);
		}
	}
}

/******************************************************************************
* Function Name  : GUI_Rect
*******************************************************************************/

void GUI_Rect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color)
{
	uint32_t index = 0;
	uint32_t row_index = 0;
...
void GUI_HLine(uint16_t y, uint16_t width, uint16_t color)
{
	uint32_t i = 0;
	const uint32_t tot = MAX_X * width;
